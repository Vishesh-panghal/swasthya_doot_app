...
class _MyRecordScreenState extends State<MyRecordScreen>
  ...
  Future<void> _generatePdf() async {
    ...
    final familiesToExport =
        _families.where((family) {
          final filteredMembers =
              family.members.where((member) {
                ...
                if (_age0to5 && (member.age < 0 || member.age > 5)) {
                  return false;
                }
                if (_age0to15 && (member.age < 0 || member.age > 15)) {
                  return false;
                }
                return true;
              }).toList();
    ...
    await Share.shareXFiles(
      [XFile(file.path)],
      subject: "Swasthya Doot Family Report",
      text: "Here is the generated family report PDF.",
    );
  }
                  ...
                  children: [
                    Expanded(
                      child: CustomTextField(
                        inData: 'Search by patient name or phone number',
                        title: '',
                    ...
                    : StreamBuilder<String>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        final filteredFamilies =
                            _families.where((family) {
                              final matchesSearch =
                                  family.head.toLowerCase().contains(search) ||
                                  family.village.toLowerCase().contains(
                                    search,
                                  ) ||
                              ...
                              final filteredMembers =
                                  family.members.where((member) {
                                    ...
                                    if (_filterSugar && !(member.isSugar)) {
                                      return false;
                                    }
                                    if (_filterAgeAbove60 && member.age <= 60) {
                                      return false;
                                    }
                                    if (_age0to5 &&
                                        (member.age < 0 || member.age > 5)) {
                                      return false;
                                    }
                                    if (_age0to15 &&
                                        (member.age < 0 || member.age > 15)) {
                                      return false;
                                    }
                                    return true;
                                  }).toList();
